...
internal sealed class ConfirmTransactionCommand : ITelegramCommand
{
    private readonly IMediator _mediator;
    private readonly ITelegramBotClient _telegramBotClient;
    private readonly long _chatId;

    public ConfirmTransactionCommand(IMediator mediator, IOptions<TelegramBotOptions> options, ITelegramBotClient telegramBotClient)
    {
        _mediator = mediator;
    ...
    {
        return command.StartsWith(CommandsConstants.Transactions.Confirm);
    }
    ...
    public async Task ProcessingAsync(TelegramUser user, CancellationToken ct = default)
    {
        ...
        if (user.MessageId.HasValue)
        {
            ...
            await _telegramBotClient.SendMessage(_chatId, $"Транзакция с Id = {transactionId} подтверждена.", cancellationToken: ct);
        }
    }
}